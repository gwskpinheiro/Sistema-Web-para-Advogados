...
class ProcessoController extends Controller
{
    ...
    {
        return view('advogado.processos.index', ['processos' => Processo::with('cliente')->get()]);
    }

    public function create(): View
    {
        return view('advogado.processos.create', ['clientes' => Cliente::all()]);
    }

    public function store(StoreProcessoRequest $request): RedirectResponse
    {
        Processo::create($request->validated());
        return redirect()->route('advogado.processos.index')->with('success', 'Processo criado com sucesso.');
    }
    ...
    {
        return view('advogado.processos.edit', [
            'processo' => $processo,
    ...
    public function update(UpdateProcessoRequest $request, Processo $processo): RedirectResponse
    {
        ...
        return redirect()->route('advogado.processos.index')->with('success', 'Processo atualizado com sucesso.');
    }

    public function destroy(Processo $processo): RedirectResponse
    {
        $processo->delete();
        return redirect()->route('advogado.processos.index')->with('success', 'Processo excluído com sucesso.');
    }
}
